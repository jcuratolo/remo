...
describe("EventStore", () => {
  ...
  test("Dispatch enqueues event for async processing", done => {
    const handler = jest.fn((stateArg, [type, payloadArg]) => {
  ...
  test("Dispatch enqueues events that yield effects for async processing", done => {
    const handler = jest.fn(({ store }, [type, payloadArg]) => {
  ...
  test("DispatchSync processes all events immediately", () => {
    ...
    const event1 = [type, payload];
    const event2 = ["type-2", payload];
    store.on(type, handler);
    store.fx("type-2", handler2);
    store.dispatchSync(event1);
    store.dispatchSync(event2);
    expect(handler).toHaveBeenCalledWith(store.state, event1);
    expect(handler2.mock.calls.length).toBe(1);
    expect(handler2.mock.calls[0][1]).toBe(event2);
  ...
  test("Dispatch effects induce dispatches after an fx event is processed", done => {
    ...
    const handler = jest.fn((state, [type, payload]) => {
